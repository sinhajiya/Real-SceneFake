...
if __name__ == '__main__':
    ...
    parser.add_argument('--FT_W2V', default=True, type=lambda x: (str(x).lower() in ['true', 'yes', '1']), help='Whether to fine-tune the W2V or not')
    ...
    model_tag = f"finetuning_{args.k_value_finetune}-shot_{prefix}_{rn}"
    if args.comment:
    ...
    if args.eval:
        model_root = (os.path.dirname(args.model_path_eval))
        ...
        #     torch.load(args.model_path_eval, map_location=device)
        # )
        state_dict = torch.load(args.model_path_eval, map_location=device)
        ...
        print("Loaded model for eval from:", args.model_path_eval)
        ...
        if not args.scenefake_eval:
            ...
            with torch.no_grad():
                ...
                evaluate_eer_segments(args, seen_loader, model, device, metrics_dir1)
                ...
                if unseen_loader is not None:
                    ...
                    evaluate_eer_segments(args, unseen_loader, model, device, metrics_dir1)
    ...
    if args.pretrained_path is None:
        raise ValueError("Pretrained model path not provided")
    ...
    print("٩(⸝⸝ᵕᴗᵕ⸝⸝)و*̣̩⋆̩*   ٩(⸝⸝ᵕᴗᵕ⸝⸝)و*̣̩⋆̩*")
    ...
    print("Trainable params BEFORE:", count_trainable(model))

    for param in model.parameters():
        param.requires_grad = False

    for param in model.conformer.classifier.parameters():
        param.requires_grad = True

    for param in model.conformer.f_attention_pool.parameters():
        param.requires_grad = True

    for param in model.conformer.b_attention_pool.parameters():
        param.requires_grad = True

    for param in model.conformer.LL.parameters():
        param.requires_grad = True
    ...
    # forward top layer
    for param in model.conformer.forward_layers[top_idx].parameters():
        param.requires_grad = True

    # backward top layer
    for param in model.conformer.backward_layers[top_idx].parameters():
        param.requires_grad = True

    print("Trainable params AFTER:", count_trainable(model))
