...
def evaluate_eer_utterance(args,loader, model, device, metrics_path):
    print("this time i am doing it ki for each segment in the utterance, first find the logits of them (_x2) then staack the logits, find softmaax, get the dcore, witj these scores i proceed to find the eer using de curve.")
    ...
    for path in logits_dict:
        ...
        # print("Chunk logits sample:", stacked[:3])
    ...
    # for path in logits_dict:
    #     print(path)
    #     stacked = torch.stack(logits_dict[path])   # [N, 2]
    #     probs = torch.softmax(stacked, dim=1)
    #     mean_probs = probs.mean(dim=0)

    #     score = mean_probs[1].item()
    #     print(stacked.shape, probs, mean_probs, score)
    #     scores.append(score)
    #     targets.append(labels_dict[path])
